...
namespace Menu {
    ...
    Menu::Menu(const IDs::IDs ID, const sf::Vector2f tamBotao, const std::string nome, const unsigned int tamFonte) :
        ...
        observadorMenu(new Observador::ObservadorMenu(this, nullptr)), mouseSelecionado(false)
    {
        if (observadorMenu == nullptr) {
            std::cout << "ERROR::Menu::MenuPrincipal::nao foi possivel criar um Observador Menu Principal" << std::endl;
            exit(1);
        }
    }
