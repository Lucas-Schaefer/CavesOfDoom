...
namespace Menu {
    MenuGameOver::MenuGameOver(Fase::Fase* fase) :
        MenuPausa(IDs::IDs::menu_game_over, "FIM DE JOGO", fase),
        texto(CAMINHO_FONTE_MENU_GAME_OVER, "", 35),
        pontuacao(CAMINHO_FONTE_MENU_GAME_OVER, "", 35)
    {
        if (fase == nullptr) {
        ...
        if (!textura->loadFromFile("imagens/Menu/caixaTexto.png")) {
            std::cout << "Menu::MenuGameOver::nao foi possivel carregar textura" << std::endl;
            exit(1);
        }
    ...
    void MenuGameOver::salvarColocacao() {
        ...
        gerenciadorArquivo.gravarConteudo("arquivos/colocacao/colocacao.txt", linha);
    }
