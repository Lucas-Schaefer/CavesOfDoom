...
namespace Observador {
    ...
    ObservadorMenu::ObservadorMenu(Menu::Menu* menu, Fase::Fase* f) :
        Observador(), menu(menu), fase(f)
    {
    ...
    void ObservadorMenu::teclaPressionada(const sf::Keyboard::Key tecla) {
        if (tecladoEspecial[tecla] == "Enter") {
            ...
            {
                pGEstado->addEstado(IDs::IDs::estado_menu_jogador);
            ...
            break;
            case (IDs::IDs::botao_fase1):
            {
                pGEstado->addEstado(IDs::IDs::jogar_fase1);
                Estado::EstadoJogar* estadoJogar = dynamic_cast<Estado::EstadoJogar*>(pGEstado->getEstadoAtual());
                estadoJogar->criarFase();
            }
            break;
            case (IDs::IDs::botao_fase2):
            {
                pGEstado->addEstado(IDs::IDs::jogar_fase2);
                Estado::EstadoJogar* estadoJogar = dynamic_cast<Estado::EstadoJogar*>(pGEstado->getEstadoAtual());
                estadoJogar->criarFase();
            }
            break;
