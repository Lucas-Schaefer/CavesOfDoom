...
namespace Gerenciador {
    ...
    GerenciadorGrafico::GerenciadorGrafico() :
        window(new sf::RenderWindow(sf::VideoMode(TELA_X, TELA_Y), "Caves of Doom")),
        camera(sf::Vector2f(TELA_X, TELA_Y)), relogio()
    {
        if (window == nullptr) {
            std::cout << "ERROR::Gerenciador::GerenciadorGrafico nao foi possivel criar uma janela grafica" << std::endl;
            exit(1);
        }
    ...
    sf::Texture GerenciadorGrafico::carregarTextura(const char* caminhoTextura) {
        ...
        if (!textura.loadFromFile(caminhoTextura)) {
            std::cout << "ERRO::Gerenciador::GerenciadorGrafico::nao foi possivel encontrar o caminho da textura - " << caminhoTextura << std::endl;
            exit(1);
        }
        return textura;
    }
    ...
    sf::Font GerenciadorGrafico::carregarFonte(const char* caminhoFonte) {
        ...
        if (!fonte.loadFromFile(caminhoFonte)) {
            throw("ERROR::Gerenciador::GerenciadorGrafico::nao foi possivel encontrar o caminho da fonte");
        }
        return fonte;
    }
