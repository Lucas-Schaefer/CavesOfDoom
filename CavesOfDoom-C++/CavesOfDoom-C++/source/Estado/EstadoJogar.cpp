...
namespace Estado {
    ...
    void EstadoJogar::criarFase(const std::string arquivoEntidades, std::vector<std::string> vectorInfoFase) {
        ...
        while (i < salvarEntidades.size()) {
            ...
            if (linha.find("=") != -1) {
                ...
                case (50):
                {
                    fase = static_cast<Fase::Fase*>(new Fase::Fase1(salvarEntidadesAux, vectorInfoFase, IDs::IDs::fase1_parte_1));
                    mapFase.insert(std::pair<IDs::IDs, Fase::Fase*>(IDs::IDs::fase1_parte_1, fase));
                    if (this->ID_FaseAtual == IDs::IDs::vazio) {
                        this->ID_FaseAtual = IDs::IDs::fase1_parte_1;
                    }
                }
                break;
                case(51):
                {
                    fase = static_cast<Fase::Fase*>(new Fase::Fase1(salvarEntidadesAux, vectorInfoFase, IDs::IDs::fase1_parte_2));
                    mapFase.insert(std::pair<IDs::IDs, Fase::Fase*>(IDs::IDs::fase1_parte_2, fase));
                    if (this->ID_FaseAtual == IDs::IDs::vazio) {
                        this->ID_FaseAtual = IDs::IDs::fase1_parte_2;
                    }
                }
                break;
                case(52):
                {
                    fase = static_cast<Fase::Fase*>(new Fase::Fase1(salvarEntidadesAux, vectorInfoFase, IDs::IDs::fase1_parte_3));
                    mapFase.insert(std::pair<IDs::IDs, Fase::Fase*>(IDs::IDs::fase1_parte_3, fase));
                    if (this->ID_FaseAtual == IDs::IDs::vazio) {
                        this->ID_FaseAtual = IDs::IDs::fase1_parte_3;
                    }
                }
                break;
                default:
        ...
        Player::Jogador* pJogador = fase->getJogador();
        if (pJogador != nullptr) {
    ...
    void EstadoJogar::criarFase() {
        if (ID == IDs::IDs::jogar_fase1) {
            Fase::Fase* fase = static_cast<Fase::Fase*>(new Fase::Fase1());
            this->ID_FaseAtual = IDs::IDs::fase1_parte_1;
   ...
   /* void EstadoJogar::mudarFase(const IDs::IDs ID_Fase) {
        ...
        if (this->ID_FaseAtual != ID_Fase) {
            mapFase[ID_FaseAtual]->removerJogadorLista();
            this->ID_FaseAtual = ID_Fase;
            Fase::Fase* fase = static_cast<Fase::Fase*>(new Fase::Fase1(this->ID_FaseAtual));
            mapFase.insert(std::pair<IDs::IDs, Fase::Fase*>(ID_FaseAtual, fase));
            mapFase[ID_FaseAtual]->mudarFase(ID_Fase_Anterior);
        }
    }*/
    ...
    void EstadoJogar::mudarEstadoObservador() {
        ...
        Player::Jogador* jogador = fase->getJogador();
        if (jogador) {
