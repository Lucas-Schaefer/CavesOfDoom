...
namespace Entidades {
    ...
    namespace Item {
        ...
        Projetil::Projetil(Personagens::Personagem* personagem) :
        ...
        Projetil::Projetil(const std::vector<std::string> atributos, Personagens::Personagem* personagem) :
            Arma(IDs::IDs::projetil_inimigo, personagem), animacao(&corpo),
            velocidade(sf::Vector2f(0.0f, 0.0f)), colidiu(true), paraEsquerda(false)
        {
            try {
        ...
        void Projetil::inicializarAnimacao() {
            if (personagem != nullptr) {
                ...
                {
                case (IDs::IDs::Demonio):
                {
                    animacao.addAnimacao("imagens/Itens/Projetil.png", "ATACA", 4, 0.12f, sf::Vector2f(5.0f, 5.0f), sf::Vector2f(tam.x / 2.0f, tam.y / 2.2f), false);
